...
export default function Edit({ attributes, setAttributes, clientId }){
  ...
  return (
    <>
      <InspectorControls>
        <PanelBody title={__('Row Settings', 'origamiui')}>
          <SelectControl
            label={__('Row', 'origamiui')}
            value={styles.base.display.row}
            options={[
              { label: '---', value: '---' },
              { label: 'row', value: 'row' },
            ]}
            onChange={(newRow) => updateStyles(`base.display.row`, newRow)}
        ...
        <PanelBody title={__('Sizing Settings', 'origamiui')} initialOpen={false}>
          ...
          >
            {(tab) => (
              <>
                <Heading style={{ marginTop: '1.5em' }}>{__(`Width & Height Settings for ${tab.title}`, 'origamiui')}</Heading>
